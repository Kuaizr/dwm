...
# 更新状态栏
refresh() {
    _icons='';_music='';_bluetooth='';_wifi='';_cpu='';_mem='';_date='';_vol='';_bat='' # 重置所有模块的状态为空
    source $tempfile                                                     # 从 temp 文件中读取模块的状态
    xsetroot -name "$_icons$_wifi$_cpu$_mem$_date$_vol$_bluetooth$_bat"      # 更新状态栏
}
...
cron() {
    ...
    while true; do
        to=()                                                            # 存放本次需要更新的模块
        [ $((i % 3)) -eq 0 ]  && to=(${to[@]} wifi)                     # 每 3秒  更新 wifi
        [ $((i % 10)) -eq 0 ]  && to=(${to[@]} bluetooth)                # 每 10秒  更新 bluetooth
...
case $1 in
    ...
    updateall|check) update icons music wifi cpu mem date vol bat bluetooth ;;
